...
class Bb15NiclaVisionBoard {
 public:
  ...
  void printSummary(Print& out) const {
    out.println("[akd1500_infer_fast] board=Nicla Vision + BB15");
    out.println("[akd1500_infer_fast] bus=Wire SDA=D11 SCL=D12");
    out.println("[akd1500_infer_fast] wiring=power_down_n=D2/PA_10 akida_cs=D7 bridge_cs=D1 akida_reset_n=D3 expander=0x43");
  }
 ...
 private:
  ...
  void releasePowerDown() {
    // D2 aliases PA_10 on Nicla Vision. On this stack it is the bridge
    // power_down_n control, so HIGH keeps the companion path active.
  ...
  void assertPowerDown() {
    // Low asserts the companion board power-down path.
    pinMode(kPowerDownPin, OUTPUT);
