...
static void kf_bfly5(kiss_fft_cpx *Fout, const size_t fstride,
  ...
  for (u = 0; u < m; ++u) {
    ...
    C_FIXDIV(*Fout4, 5);
    scratch[0] = *Fout0;
    C_MUL(scratch[1], *Fout1, st->twiddles[u * fstride]);
    C_MUL(scratch[2], *Fout2, st->twiddles[2 * u * fstride]);
    C_MUL(scratch[3], *Fout3, st->twiddles[3 * u * fstride]);
    C_MUL(scratch[4], *Fout4, st->twiddles[4 * u * fstride]);
