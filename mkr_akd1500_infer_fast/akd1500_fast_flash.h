...
struct Config {
  ...
  // DODANO ZA MKR MAPIRANJE PIN_OVA:
  uint8_t akidaCsPin = 20u;
  uint8_t bridgeCsPin = 13u;
};
...
inline AKD1500Options make_options(const Config& config) {
  ...
  // PREPISIVANJE CS PIN_OVA ZA MKR:
  options.akidaCsPin = config.akidaCsPin;
  options.bridgeCsPin = config.bridgeCsPin;
  return options;
}
...
// 1. Zamijeni printLastError da bude prazna
  void printLastError(Print& out) const {
    // Prazno radi uštede memorije
  }

  // 2. Zamijeni printModelInfo da bude prazna
  void printModelInfo(Print& out) const {
    // Prazno radi uštede memorije
  }

  // 3. Osiguraj da je i ova potpuno prazna
