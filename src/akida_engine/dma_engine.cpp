...
#ifndef AKIDA_NICLA_DMA_LAUNCH_TRACE
#define AKIDA_NICLA_DMA_LAUNCH_TRACE 0
#endif

#ifndef AKIDA_NICLA_HRC_POST_KICK_QUIET_US
#define AKIDA_NICLA_HRC_POST_KICK_QUIET_US 0
#endif

#ifndef AKIDA_NICLA_CONFIG_POST_DESC_QUIET_US
#define AKIDA_NICLA_CONFIG_POST_DESC_QUIET_US 5000
#endif

#ifndef AKIDA_NICLA_CONFIG_POST_ENABLE_QUIET_US
#define AKIDA_NICLA_CONFIG_POST_ENABLE_QUIET_US 5000
#endif

#ifndef AKIDA_NICLA_CONFIG_TRACE_EQUIV_GAP_US
#define AKIDA_NICLA_CONFIG_TRACE_EQUIV_GAP_US 0
#endif

namespace {

constexpr uint32_t kTopRegsBase = 0xFCC00000u;
constexpr uint32_t kControlSignalsReg = 0xFCE00018u;
constexpr uint32_t kHrcDmaBase = 0xFCC28000u;
constexpr uint32_t kConfigDmaBase = 0xFCC30000u;

inline void nicla_quiet_wait_us(uint32_t delay_us) {
#ifdef ARDUINO
...
inline void nicla_config_trace_equiv_gap(const Engine& dma) {
#if !AKIDA_NICLA_DMA_ENQUEUE_TRACE
  if (AKIDA_NICLA_CONFIG_TRACE_EQUIV_GAP_US > 0 &&
      dma.reg_base_addr == kConfigDmaBase) {
    nicla_quiet_wait_us(AKIDA_NICLA_CONFIG_TRACE_EQUIV_GAP_US);
  }
#else
  (void)dma;
#endif
...
inline void dump_dma_enqueue_snapshot(HardwareDriver* driver, const Engine& dma,
                                      const char* where) {
#if AKIDA_NICLA_DMA_ENQUEUE_TRACE
  const uint32_t base = dma.reg_base_addr;
  std::printf(
      "[AKIDA][DMA_ENQ_SNAP] where=%s base=0x%08lX ctrl=0x%08lX desc_cont=0x%08lX cont_size=0x%08lX desc_status=0x%08lX mon_ctrl=0x%08lX mon_status=0x%08lX job_fifo=0x%08lX timer=0x%08lX\r\n",
      ...
      static_cast<unsigned long>(driver->read32(base + DMA_CONT_SIZE_REG)),
...
inline void dump_dma_enqueue_top_snapshot(HardwareDriver* driver,
                                          const char* where) {
#if AKIDA_NICLA_DMA_ENQUEUE_TRACE
  std::printf(
      "[AKIDA][DMA_ENQ_TOP] where=%s ctrl_signals=0x%08lX general_control=0x%08lX int_glb=0x%08lX int_mask=0x%08lX int_src=0x%08lX\r\n",
      where ? where : "unknown",
      static_cast<unsigned long>(driver->read32(kControlSignalsReg)),
      static_cast<unsigned long>(driver->read32(kTopRegsBase + REG_GENERAL_CONTROL)),
      static_cast<unsigned long>(
          driver->read32(kTopRegsBase + REG_INTERRUPT_CONTROLLER_GENERAL_CONTROL)),
      static_cast<unsigned long>(
          driver->read32(kTopRegsBase + REG_INTERRUPT_CONTROLLER_SOURCE_MASK)),
      static_cast<unsigned long>(
          driver->read32(kTopRegsBase + REG_INTERRUPT_CONTROLLER_SOURCE)));
#else
  (void)driver;
  (void)where;
#endif
...
inline void dump_descriptor_words(const Engine& dma, dma::addr descriptor_addr,
                                  const Descriptor& descriptor,
                                  const char* where) {
#if AKIDA_NICLA_DMA_ENQUEUE_TRACE
  if (dma.reg_base_addr != kHrcDmaBase) {
    return;
  }
  std::printf(
      "[AKIDA][DMA_DESC] where=%s base=0x%08lX addr=0x%08lX words=%lu\r\n",
      where ? where : "unknown",
      static_cast<unsigned long>(dma.reg_base_addr),
      static_cast<unsigned long>(descriptor_addr),
      static_cast<unsigned long>(descriptor.size()));
  for (size_t i = 0; i < descriptor.size(); i += 8) {
    std::printf(
        "[AKIDA][DMA_DESC] where=%s chunk=%lu data=",
        where ? where : "unknown", static_cast<unsigned long>(i / 8));
    for (size_t j = i; j < descriptor.size() && j < i + 8; ++j) {
      std::printf("%s0x%08lX", (j == i) ? "" : ",",
                  static_cast<unsigned long>(descriptor[j]));
    }
    std::printf("\r\n");
  }
#else
  (void)dma;
  (void)descriptor_addr;
  (void)descriptor;
  (void)where;
#endif
}

inline void dump_readback_words(HardwareDriver* driver, uint32_t address,
                                size_t num_words, const char* where,
                                const char* kind) {
#if AKIDA_NICLA_DMA_ENQUEUE_TRACE
  if (!driver || num_words == 0) {
    return;
  }
  std::vector<uint32_t> words(num_words, 0);
  driver->read(address, words.data(), num_words * sizeof(uint32_t));
  std::printf(
      "[AKIDA][DMA_READBACK] where=%s kind=%s addr=0x%08lX words=%lu\r\n",
      where ? where : "unknown", kind ? kind : "unknown",
      static_cast<unsigned long>(address),
      static_cast<unsigned long>(num_words));
  for (size_t i = 0; i < words.size(); i += 8) {
    std::printf(
        "[AKIDA][DMA_READBACK] where=%s kind=%s chunk=%lu data=",
        where ? where : "unknown", kind ? kind : "unknown",
        static_cast<unsigned long>(i / 8));
    for (size_t j = i; j < words.size() && j < i + 8; ++j) {
      std::printf("%s0x%08lX", (j == i) ? "" : ",",
                  static_cast<unsigned long>(words[j]));
    }
    std::printf("\r\n");
  }
#else
...
static void reset_engine(HardwareDriver* driver, uint32_t reg_base_addr) {
  ...
  nicla_quiet_wait_us(20u);
  driver->write32(reg_base_addr + DMA_CTRL_REG, 0u);
  nicla_quiet_wait_us(20u);
}
...
static bool wait_for_interrupt_ext(HardwareDriver* driver, const Engine& dma,
  ...
  // Fallback guard in case system tick is not advancing while polling.
  constexpr uint32_t kMaxSpinsWithoutTick = 5000000u;
  auto start = time_ms();
  auto last_tick = start;
  uint32_t spins_without_tick = 0;
  ...
  while (true) {
    ...
    if (now != last_tick) {
      last_tick = now;
      spins_without_tick = 0;
    } else {
      ++spins_without_tick;
      if (spins_without_tick > kMaxSpinsWithoutTick) {
        const auto status =
            driver->read32(dma.reg_base_addr + DMA_BUF_MON_STATUS_REG);
        const auto ctrl =
            driver->read32(dma.reg_base_addr + DMA_IB_BUF_MON_CTRL_REG);
        std::printf(
            "[AKIDA][DMA] wait stalled base=0x%08lX status=0x%08lX "
            "ctrl=0x%08lX flag_off=%lu flag_bits=%lu spins=%lu\r\n",
            static_cast<unsigned long>(dma.reg_base_addr),
            static_cast<unsigned long>(status),
            static_cast<unsigned long>(ctrl),
            static_cast<unsigned long>(flag.offset),
            static_cast<unsigned long>(flag.nb_bits),
            static_cast<unsigned long>(spins_without_tick));
        return false;
      }
    }

    if ((now - start) > kTimeoutMs) {
      const auto status = driver->read32(dma.reg_base_addr + DMA_BUF_MON_STATUS_REG);
      const auto ctrl = driver->read32(dma.reg_base_addr + DMA_IB_BUF_MON_CTRL_REG);
      std::printf(
          "[AKIDA][DMA] wait timeout base=0x%08lX elapsed_ms=%ld "
          "status=0x%08lX ctrl=0x%08lX flag_off=%lu flag_bits=%lu\r\n",
          static_cast<unsigned long>(dma.reg_base_addr),
          static_cast<long>(now - start),
          static_cast<unsigned long>(status),
          static_cast<unsigned long>(ctrl),
          static_cast<unsigned long>(flag.offset),
          static_cast<unsigned long>(flag.nb_bits));
...
dma::addr enqueue_descriptor(HardwareDriver* driver, const Engine& dma,
  ...
  nicla_config_trace_equiv_gap(dma);
  dump_dma_enqueue_snapshot(driver, dma, "enqueue_descriptor:entry");
  ...
  if (dma.reg_base_addr == kHrcDmaBase && AKIDA_NICLA_DMA_LAUNCH_TRACE) {
    std::printf(
        "[AKIDA][DMA_LAUNCH] platform=nicla where=enqueue_descriptor:desc_cont_read addr=0x%08lX value=0x%08lX\r\n",
        static_cast<unsigned long>(desc_container_reg_addr),
  ...
  nicla_config_trace_equiv_gap(dma);
  dump_descriptor_words(dma, last_descriptor_addr, descriptor,
                        "enqueue_descriptor:descriptor");
  dump_dma_enqueue_snapshot(driver, dma, "enqueue_descriptor:pre_desc_write");
  driver->write(last_descriptor_addr, descriptor.data(),
                descriptor.size() * sizeof(Descriptor::value_type));
  nicla_config_trace_equiv_gap(dma);
  dump_dma_enqueue_snapshot(driver, dma, "enqueue_descriptor:post_desc_write");
  ...
  if (dma.reg_base_addr == kHrcDmaBase && !descriptor.empty()) {
    dump_readback_words(driver, last_descriptor_addr, descriptor.size(),
                        "enqueue_descriptor:descriptor_readback",
                        "descriptor");
    dump_readback_words(driver, descriptor[hrc::DESC_INPUT_ADDR], 8u,
                        "enqueue_descriptor:payload_head_readback",
                        "payload_head");
  }
  nicla_config_trace_equiv_gap(dma);
  dump_dma_enqueue_snapshot(driver, dma, "enqueue_descriptor:pre_last_desc_write");
  if (dma.reg_base_addr == kHrcDmaBase && AKIDA_NICLA_DMA_LAUNCH_TRACE) {
    std::printf(
        "[AKIDA][DMA_LAUNCH] platform=nicla where=enqueue_descriptor:desc_cont_write addr=0x%08lX value=0x%08lX\r\n",
        static_cast<unsigned long>(desc_container_reg_addr),
        static_cast<unsigned long>(reg));
  }
  driver->write32(desc_container_reg_addr, reg);
  if (dma.reg_base_addr == kHrcDmaBase &&
      AKIDA_NICLA_HRC_POST_KICK_QUIET_US > 0) {
    nicla_quiet_wait_us(AKIDA_NICLA_HRC_POST_KICK_QUIET_US);
  } else if (dma.reg_base_addr == kConfigDmaBase) {
    nicla_quiet_wait_us(AKIDA_NICLA_CONFIG_POST_DESC_QUIET_US);
  }
  nicla_config_trace_equiv_gap(dma);
  dump_dma_enqueue_snapshot(driver, dma, "enqueue_descriptor:post_last_desc_write");
  dump_dma_enqueue_top_snapshot(driver, "enqueue_descriptor:post_last_desc_top");
  nicla_config_trace_equiv_gap(dma);
  dump_dma_enqueue_snapshot(driver, dma, "enqueue_descriptor:pre_return");
  dump_dma_enqueue_top_snapshot(driver, "enqueue_descriptor:pre_return_top");
...
bool wait_config_dma_descriptor_complete(HardwareDriver* driver,
  ...
  if (dma.engine.reg_base_addr == kConfigDmaBase) {
    nicla_quiet_wait_us(AKIDA_NICLA_CONFIG_POST_ENABLE_QUIET_US);
  }
