...
static dma::wbuffer format_cnp_events(const Tensor& inputs) {
  if (inputs.tensor_class() == Tensor::Class::Sparse) {
    return format_cnp_events(static_cast<const Sparse&>(inputs));
  }
  return format_cnp_events(static_cast<const Dense&>(inputs));
}

static dma::wbuffer format_fnp_events(const Tensor& inputs) {
  if (inputs.tensor_class() == Tensor::Class::Sparse) {
    return format_fnp_events(static_cast<const Sparse&>(inputs));
  }
  return format_fnp_events(static_cast<const Dense&>(inputs));
}
...
TensorUniquePtr dma_events_read_outputs(HardwareDriver* driver,
  ...
  // Output payload is bounded by the buffer allocated from program metadata.
  // Guard against lane-swapped/corrupted header size to avoid very long SPI
  // reads that look like a fetch hang.
  const size_t output_slot_bytes = output_memory_required(program_info);
  if (output_slot_bytes < dma::kOutputHeaderByteSize) {
    panic("Invalid output slot size: %lu bytes",
          static_cast<unsigned long>(output_slot_bytes));
  }
  const size_t max_payload_bytes = output_slot_bytes - dma::kOutputHeaderByteSize;
  const uint32_t max_output_words =
      static_cast<uint32_t>(max_payload_bytes / sizeof(dma::w32));
  if (output_word_size > max_output_words) {
    panic("Invalid output header word count");
  }
