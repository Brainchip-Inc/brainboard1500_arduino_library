...
const Sparse* as_sparse(const Tensor& input) {
  if (input.tensor_class() == Tensor::Class::Sparse) {
    return static_cast<const Sparse*>(&input);
  }
  return nullptr;
}
...
const Dense* as_dense(const Tensor& input) {
  if (input.tensor_class() == Tensor::Class::Dense) {
    return static_cast<const Dense*>(&input);
  }
  return nullptr;
}
